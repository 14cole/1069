...
def _load_2d_rcs_grim(path: str) -> Dict[str, Rcs2DTable]:
    ...
    with np.load(path, allow_pickle=False) as data:
        ...
        raw_amp_real = np.asarray(data["rcs_amp_real"], dtype=float) if "rcs_amp_real" in data else None
        raw_amp_imag = np.asarray(data["rcs_amp_imag"], dtype=float) if "rcs_amp_imag" in data else None
    ...
    if raw_amp_real is not None and raw_amp_real.ndim != 4:
        raise ValueError(f"GRIM file '{path}' has unexpected rcs_amp_real rank {raw_amp_real.ndim}; expected 4D")
    if raw_amp_imag is not None and raw_amp_imag.ndim != 4:
        raise ValueError(f"GRIM file '{path}' has unexpected rcs_amp_imag rank {raw_amp_imag.ndim}; expected 4D")
    ...
    if raw_amp_real is not None and raw_amp_imag is not None:
        raw_amp = np.asarray(raw_amp_real, dtype=np.float64) + 1j * np.asarray(raw_amp_imag, dtype=np.float64)
        raw_valid = np.isfinite(raw_amp.real) & np.isfinite(raw_amp.imag)
        if np.any(raw_valid):
            rcs_amp[raw_valid] = raw_amp[raw_valid]
    amp_valid = np.isfinite(rcs_power) & np.isfinite(rcs_phase) & ~np.isfinite(rcs_amp.real)
    if np.any(amp_valid):
...
def _compute_sigma3d_for_direction(
    ...
    sigma_density_local = np.zeros(len(points.weights), dtype=float)
    ...
    for i, ang in enumerate(angles):
        ...
        s3_density = 2.0 * s2 / max(lam, 1e-15)
        sigma_density_local[i] = s3_density
        sigma_local[i] = s3_density * float(points.weights[i])
    ...
    if combine_mode in {"coherent_zero_phase", "coherent_2d_phase"}:
        ...
        # Coherent integration uses amplitude density integrated linearly in weight:
        # a_i = sqrt(sigma_density_i) * weight_i * exp(j*phase_i)
        amp = (
            np.sqrt(np.maximum(sigma_density_local, 0.0))
            * np.asarray(points.weights, dtype=float)
            * phase
        )
