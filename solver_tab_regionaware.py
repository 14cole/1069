...
from geometry_io_regionaware import build_geometry_snapshot, parse_geometry
...
class _SolveWorker(QObject):
    ...
    def __init__(
        ...
        self.basis_family = str(basis_family)
        self.testing_family = str(testing_family)
        ...
        try:
            if self.mesh_convergence:
                ...
                result = solve_monostatic_rcs_2d(
                    ...
                    testing_family=self.testing_family,
                )
...
class SolverTab(QWidget):
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        self.cmb_solve_method = QComboBox()
        self.cmb_solve_method.addItem("Legacy: Pulse / Collocation", userData=("pulse", "collocation"))
        self.cmb_solve_method.addItem("New: Linear / Galerkin", userData=("linear", "galerkin"))
        ...
        options_form.addRow("Solve Method", self.cmb_solve_method)
    ...
    def _load_geometry_for_solver(self) -> Tuple[Dict[str, Any], str, str]:
        ...
        if path:
            ...
            snapshot = build_geometry_snapshot(title, segments, ibcs_entries, dielectric_entries, region_defs)
    ...
    def _set_solving_state(self, solving: bool):
        ...
        self.cmb_solve_method.setEnabled(not solving)
    ...
    def _run_solver(self):
        ...
        try:
            ...
            method_pair = self.cmb_solve_method.currentData()
            if not isinstance(method_pair, tuple) or len(method_pair) != 2:
                raise ValueError("Invalid solve method selection.")
            basis_family, testing_family = str(method_pair[0]), str(method_pair[1])
