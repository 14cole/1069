...
def _solve_one_frequency(
    ...
    rcs_normalization_mode: str | None,
    apply_rcs_normalization: bool | None,
    rcs_normalization_method: str,
    rcs_normalization_numerator: float | None,
    impedance_definition: str,
    require_strong_special_functions_for_lossy: bool,
) -> Dict[str, Any]:
    return solve_monostatic_rcs_2d(
        ...
        apply_rcs_normalization=(
            True if apply_rcs_normalization is None else bool(apply_rcs_normalization)
        ),
        rcs_normalization_method=str(rcs_normalization_method),
        rcs_normalization_numerator=rcs_normalization_numerator,
        impedance_definition=str(impedance_definition),
        require_strong_special_functions_for_lossy=bool(require_strong_special_functions_for_lossy),
    )
...
def run_headless(
    ...
    rcs_normalization_mode: str | None = "divide_by_k",
    apply_rcs_normalization: bool | None = None,
    rcs_normalization_method: str = "divide_by_k",
    rcs_normalization_numerator: float | None = None,
    impedance_definition: str = "surface_impedance_ohms",
    require_strong_special_functions_for_lossy: bool = False,
    ...
    if rcs_normalization_numerator is not None and float(rcs_normalization_numerator) <= 0.0:
        raise ValueError("rcs_normalization_numerator must be > 0 when provided.")
            ...
            with ProcessPoolExecutor(max_workers=workers) as ex:
                futures = {
                    ex.submit(
                        ...
                        rcs_normalization_mode,
                        apply_rcs_normalization,
                        rcs_normalization_method,
                        rcs_normalization_numerator,
                        impedance_definition,
                        require_strong_special_functions_for_lossy,
                    ): float(freq)
    ...
    grim_files = export_result_to_grim(
        ...
        preserve_raw_complex_amplitude=bool(preserve_raw_complex_amplitude),
    )
    ...
    payload = {
        ...
        "headless_settings": {
            "rcs_normalization_mode": rcs_normalization_mode,
            "apply_rcs_normalization": apply_rcs_normalization,
            "rcs_normalization_method": rcs_normalization_method,
            "rcs_normalization_numerator": rcs_normalization_numerator,
            "impedance_definition": impedance_definition,
            "require_strong_special_functions_for_lossy": bool(require_strong_special_functions_for_lossy),
            "preserve_raw_complex_amplitude": bool(preserve_raw_complex_amplitude),
        },
    }
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--rcs-normalization-mode",
        default="divide_by_k",
        choices=["divide_by_k", "no_k", "off"],
        help="Legacy RCS post-processing normalization mode alias.",
    )
    parser.add_argument(
        "--disable-rcs-normalization",
        action="store_true",
        help="Disable final amplitude-to-sigma normalization and report raw |A|^2.",
    )
    parser.add_argument(
        "--rcs-normalization-method",
        default="divide_by_k",
        choices=["divide_by_k", "constant"],
        help="Normalization formula when normalization is enabled.",
    )
    parser.add_argument(
        "--rcs-normalization-numerator",
        type=float,
        default=None,
        help="Optional numerator for normalization formula. Defaults to solver default.",
    )
    parser.add_argument(
        "--impedance-definition",
        default="surface_impedance_ohms",
        choices=["surface_impedance_ohms", "reduced_robin"],
        help="Interpretation of impedance-card values.",
    )
    parser.add_argument(
        "--require-strong-special-functions-for-lossy",
        action="store_true",
        help="Fail lossy coupled solves unless SciPy or mpmath complex Hankel support is available.",
    )
    parser.add_argument(
        "--no-preserve-raw-complex-amplitude",
        action="store_true",
        help="Do not store raw solver complex far-field amplitude arrays in exported .grim files.",
    )
    parser.add_argument(
        "--json-summary",
        default="",
...
def main(argv: List[str] | None = None) -> int:
    ...
    payload = run_headless(
        ...
        rcs_normalization_mode=str(args.rcs_normalization_mode),
        apply_rcs_normalization=(False if bool(args.disable_rcs_normalization) else None),
        rcs_normalization_method=str(args.rcs_normalization_method),
        rcs_normalization_numerator=args.rcs_normalization_numerator,
        impedance_definition=str(args.impedance_definition),
        require_strong_special_functions_for_lossy=bool(args.require_strong_special_functions_for_lossy),
        preserve_raw_complex_amplitude=not bool(args.no_preserve_raw_complex_amplitude),
    )
    ...
    summary = {
        ...
        "headless_settings": payload.get("headless_settings", {}),
    }
