...
class Panel:
    ...
    normal: np.ndarray
    length: float
    plus_region_override: int | None = None
    minus_region_override: int | None = None
...
class MaterialLibrary:
    """Material lookup facade for constant values, region->material mappings, and fort.* frequency tables."""

    def __init__(
        self,
        impedance_models: Dict[int, complex | ComplexTable],
        dielectric_models: Dict[int, Tuple[complex, complex] | MediumTable],
        region_material_flags: Dict[int, int] | None = None,
    ):
        self.impedance_models = impedance_models
        self.dielectric_models = dielectric_models
        self.region_material_flags = {int(k): int(v) for k, v in (region_material_flags or {}).items()}
    ...
    def from_entries(
        ...
        base_dir: str,
        region_entries: List[Dict[str, Any]] | List[List[str]] | None = None,
    ) -> "MaterialLibrary":
        impedance_models: Dict[int, complex | ComplexTable] = {}
        dielectric_models: Dict[int, Tuple[complex, complex] | MediumTable] = {}
        region_material_flags: Dict[int, int] = {}
        ...
        for row in list(region_entries or []):
            if isinstance(row, dict):
                region_id = _parse_flag(row.get("region_id", 0))
                material_flag = _parse_flag(row.get("material_flag", 0))
            else:
                vals = list(row)
                region_id = _parse_flag(vals[0] if len(vals) > 0 else 0)
                material_flag = _parse_flag(vals[1] if len(vals) > 1 else 0)
            if region_id > 0:
                region_material_flags[int(region_id)] = int(material_flag)

        return cls(
            impedance_models=impedance_models,
            dielectric_models=dielectric_models,
            region_material_flags=region_material_flags,
        )
    ...
    def get_region_material_flag(self, region_flag: int) -> int:
        if region_flag <= 0:
            return 0
        return int(self.region_material_flags.get(int(region_flag), int(region_flag)))

    def get_region_medium(self, region_flag: int, freq_ghz: float) -> Tuple[complex, complex]:
        material_flag = self.get_region_material_flag(region_flag)
        return self.get_medium(material_flag, freq_ghz)
...
def validate_geometry_snapshot_for_solver(
    ...
    region_rows = list(geometry_snapshot.get('regions', []) or [])
    ...
    for row in region_rows:
        if isinstance(row, dict):
            rid = _parse_flag(row.get('region_id', 0))
            mflag = _parse_flag(row.get('material_flag', 0))
        else:
            vals = list(row)
            rid = _parse_flag(vals[0] if len(vals) > 0 else 0)
            mflag = _parse_flag(vals[1] if len(vals) > 1 else 0)
        if rid > 0:
            region_to_material[int(rid)] = int(mflag)
    ...
    for seg_idx, seg in enumerate(segments):
        ...
        plus_region = None if seg.get('plus_region', None) in (None, '') else _parse_flag(seg.get('plus_region'))
        minus_region = None if seg.get('minus_region', None) in (None, '') else _parse_flag(seg.get('minus_region'))
        ...
        if seg_type == 3:
            eff_minus = int(minus_region) if minus_region is not None else int(ipn1)
            eff_plus = int(plus_region) if plus_region is not None else 0
            if eff_minus <= 0:
                raise ValueError(f"TYPE 3 segment '{seg_name}' requires a dielectric on the minus side via IPN1 or explicit minus_region.")
            if eff_plus != 0:
                warnings.append(f"TYPE 3 segment '{seg_name}' uses plus_region={eff_plus}; expected ambient air (0) unless intentionally overridden.")
            mat_minus = region_to_material.get(eff_minus, eff_minus)
            if mat_minus > 50:
                _resolve_fort_file(base_dir, mat_minus)
            elif mat_minus not in diel_flags:
                raise ValueError(f"TYPE 3 segment '{seg_name}' references undefined dielectric/material flag {mat_minus} on the minus side.")
        elif seg_type == 4:
            if ipn1 <= 0:
...
def _build_panels(
    ...
    for seg in segments:
        ...
        plus_region_raw = seg.get("plus_region", None)
        minus_region_raw = seg.get("minus_region", None)
        plus_region_override = None if plus_region_raw in (None, "") else _parse_flag(plus_region_raw)
        minus_region_override = None if minus_region_raw in (None, "") else _parse_flag(minus_region_raw)
            ...
            for i in range(count):
                ...
                panels.append(
                    Panel(
                        ...
                        normal=normal,
                        length=length,
                        plus_region_override=plus_region_override,
                        minus_region_override=minus_region_override,
                    )
                )
...
def prepare_linear_galerkin_foundation(
    ...
    materials = MaterialLibrary.from_entries(
        ...
        base_dir=base_dir,
        region_entries=geometry_snapshot.get("regions", []) or [],
    )
...
def _region_medium(materials: MaterialLibrary, region_flag: int, freq_ghz: float) -> Tuple[complex, complex]:
    if region_flag <= 0:
        return 1.0 + 0.0j, 1.0 + 0.0j
    return materials.get_region_medium(region_flag, freq_ghz)
...
def _build_coupled_panel_info(
    ...
    Project convention:
    - the drawn panel normal points toward the plus side,
    - TYPE 3: plus = air, minus/IPN1 = dielectric,
    - TYPE 5: plus/IPN1, minus/IPN2 unless explicit region overrides are provided,
    - TYPE 4: plus/IPN1 = dielectric, minus = PEC/IBC side unless explicit overrides are provided.
    ...
    for panel in panels:
        seg_type = panel.seg_type
        if seg_type == 3:
            if panel.ipn1 <= 0 and panel.minus_region_override is None:
                raise ValueError(f"TYPE 3 panel '{panel.name}' requires IPN1 > 0 or an explicit minus region.")
            plus_region = int(panel.plus_region_override) if panel.plus_region_override is not None else 0
            minus_region = int(panel.minus_region_override) if panel.minus_region_override is not None else int(panel.ipn1)
            bc_kind = "transmission"
            plus_has_incident = plus_region == 0
            minus_has_incident = minus_region == 0
        elif seg_type == 5:
            if (panel.plus_region_override is None and panel.ipn1 <= 0) or (panel.minus_region_override is None and panel.ipn2 <= 0):
                raise ValueError(f"TYPE 5 panel '{panel.name}' requires plus/minus regions via IPN1/IPN2 or explicit region overrides.")
            plus_region = int(panel.plus_region_override) if panel.plus_region_override is not None else int(panel.ipn1)
            minus_region = int(panel.minus_region_override) if panel.minus_region_override is not None else int(panel.ipn2)
            bc_kind = "transmission"
            plus_has_incident = plus_region == 0
            minus_has_incident = minus_region == 0
        elif seg_type == 4:
            if panel.ipn1 <= 0 and panel.plus_region_override is None:
                raise ValueError(f"TYPE 4 panel '{panel.name}' requires IPN1 > 0 or an explicit plus region.")
            plus_region = int(panel.plus_region_override) if panel.plus_region_override is not None else int(panel.ipn1)
            minus_region = int(panel.minus_region_override) if panel.minus_region_override is not None else -1
            bc_kind = "robin"
            plus_has_incident = plus_region == 0
            minus_has_incident = minus_region == 0
        elif seg_type == 2:
            minus_region = int(panel.minus_region_override) if panel.minus_region_override is not None else 0
            plus_region = int(panel.plus_region_override) if panel.plus_region_override is not None else -1
            bc_kind = "robin"
            minus_has_incident = minus_region == 0
            plus_has_incident = plus_region == 0
        elif seg_type == 1:
            ...
            if sheet_region is None:
                ...
                next_sheet_region += 1
            minus_region = int(panel.minus_region_override) if panel.minus_region_override is not None else 0
            plus_region = int(panel.plus_region_override) if panel.plus_region_override is not None else sheet_region
            bc_kind = "transmission"
            minus_has_incident = minus_region == 0
            plus_has_incident = plus_region == 0
        else:
