...
ADVANCED: Dict[str, Any] = {
    ...
    # RCS normalization / export controls
    # - rcs_normalization_mode is the legacy alias still accepted by the solver
    # - set apply_rcs_normalization=False to export raw |A|^2 instead
    "rcs_normalization_mode": "no_k",
    "apply_rcs_normalization": None,  # None = use legacy alias above
    "rcs_normalization_method": "divide_by_k",
    "rcs_normalization_numerator": None,
    "impedance_definition": "surface_impedance_ohms",
    "require_strong_special_functions_for_lossy": False,
...
def main() -> int:
    ...
    payload = run_headless(
        ...
        rcs_normalization_mode=(None if advanced.get("rcs_normalization_mode", None) is None else str(advanced.get("rcs_normalization_mode", "divide_by_k"))),
        apply_rcs_normalization=advanced.get("apply_rcs_normalization", None),
        rcs_normalization_method=str(advanced.get("rcs_normalization_method", "divide_by_k")),
        rcs_normalization_numerator=advanced.get("rcs_normalization_numerator", None),
        impedance_definition=str(advanced.get("impedance_definition", "surface_impedance_ohms")),
        require_strong_special_functions_for_lossy=bool(advanced.get("require_strong_special_functions_for_lossy", False)),
        preserve_raw_complex_amplitude=bool(advanced.get("preserve_raw_complex_amplitude", True)),
    )
